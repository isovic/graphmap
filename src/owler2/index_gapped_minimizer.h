...
const uint32_t kIndexMaskStrand = ~((uint32_t) 1);
...
class IndexPos {
  ...
  IndexPos(uint64_t coded_pos) {
    id = coded_pos >> 32;
    pos = coded_pos & (0xFFFFFFFF00000000);
  }
...
//  std::vector<int64_t> seq_seed_counts_;
  ...
  uint64_t SeedHashFunction_(uint64_t seed);
  uint64_t ReverseComplementSeed_(uint64_t seed, int32_t num_bases);
