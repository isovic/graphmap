...
void OwlerData::Init(SingleSequence *read, std::vector<std::shared_ptr<is::MinimizerIndex>> &indexes) {
  read_ = read;
//  indexes_ = &indexes;
  indexes_ = indexes;
  overlaps.clear();
...
//  }
  ...
  for (int64_t i = 0; i < indexes.size(); i++) {
    for (int32_t j=0; j<indexes[i]->get_index_shapes().size(); j++) {
      seed_types.push_back(indexes[i]->get_index_shapes()[j].shape());
    }
  }
}
