...
int Owler::ProcessRead(OwlerData *owler_data, std::vector<std::shared_ptr<is::MinimizerIndex>> indexes, const SingleSequence *read, const ProgramParameters *parameters, const EValueParams *evalue_params) {
...
int Owler::CollectSeedHits(OwlerData* owler_data, std::vector<std::shared_ptr<is::MinimizerIndex>> &indexes, const SingleSequence* read, const ProgramParameters* parameters) {
  ...
  for (int64_t i = 0; i < readlength; i += parameters->kmer_step) {
    ...
    for (int64_t j = 0; j < indexes.size(); j++) {
      ...
      int64_t k = index->get_shape_max_width();
      if ((i + k) >= readlength)
        continue;
...
int Owler::FilterUnlikelyOverlaps(OwlerData* owler_data, std::vector<std::shared_ptr<is::MinimizerIndex>> &indexes, const SingleSequence* read, const ProgramParameters* parameters) {
...
int Owler::ApplyLCS(OwlerData* owler_data, std::vector<std::shared_ptr<is::MinimizerIndex>> &indexes, const SingleSequence* read, const ProgramParameters* parameters) {
  for (int64_t i = 0; i < owler_data->overlaps.size(); i++) {
...
std::string Owler::OverlapMHAPVerbose(OwlerData* owler_data, std::vector<std::shared_ptr<is::MinimizerIndex>> &indexes, const SingleSequence* read, const ProgramParameters* parameters, int64_t ref_id, int64_t hits_start, std::vector<int> &lcskpp_indices) {
  std::stringstream ss;
...
int Owler::ApplyLCS2(OwlerData* owler_data, std::vector<std::shared_ptr<is::MinimizerIndex>> &indexes, const SingleSequence* read, const ProgramParameters* parameters) {
...
int Owler::FilterAnchorBreakpoints(const std::vector<int> &lcskpp_indices, int64_t ref_hits_start, int64_t ref_hits_end, int64_t seed_length, int64_t min_cluster_length, float min_cluster_coverage, OwlerData* owler_data, std::vector<std::shared_ptr<is::MinimizerIndex>> &indexes, const SingleSequence* read, const ProgramParameters* parameters, std::vector<int> &ret_filtered_lcskpp_indices, std::vector<int32_t> *ret_cluster_ids) {
