...
struct ProgramParameters {
  ...
  // Finalization.
  bool filter_by_edit_distance = false;      // 'f' If a read passed all the tests, it's edit distance is still measured at the end. If true, the edit distance has to be lower than a certain threshold to call a read 'mapped'.
...
//#define DEFAULT_OUTPUT_MULTIPLE_ALIGNMENTS  (bool) false
//
...
//  bool output_multiple_alignments = DEFAULT_OUTPUT_MULTIPLE_ALIGNMENTS;  // If 0, only one best alignment will be output. Otherwise, all alignments within margin_for_ambiguity will be output to a file.
//
