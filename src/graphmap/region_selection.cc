...
int GraphMap::RegionSelectionSpacedHash_(int64_t bin_size, MappingData* mapping_data, const IndexSpacedHash* index, const IndexSpacedHash* index_secondary, const SingleSequence* read, const ProgramParameters* parameters) {
...
// Ovaj debug sam maknuo za brzinu kod profiliranja!
  if (parameters->verbose_level > 8 && read->get_sequence_id() == parameters->debug_read) {
    ...
    // The tuple will contain: reference_id, bin_index, bin_count.
    for (int64_t i = 0; i < (index->get_num_sequences_forward()); i++) {
      LogSystem::GetInstance().Log(VERBOSE_LEVEL_ALL_DEBUG, read->get_sequence_id() == parameters->debug_read, FormatString("[ref %ld] (forward) %s, bins_chromosome.size() = %ld, ", i, index->get_headers()[i].c_str(), bins_chromosome[i].size()), "[]");
